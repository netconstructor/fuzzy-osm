    ...
    if elem.tag == "node":
      if tags:
        ...
        if tags != ntags:
          ...
          osmcode.write( '<node id="%s" uid="%s" user="%s" timestamp="%s" lon="%s" lat="%s" version="%s" action="modify">\n'%(items["id"], items.get("uid",0), xml_escape((items.get("user", "unknown"),))[0], items["timestamp"], items["lon"],items["lat"],items["version"]))
      ...
      if tags:
        ...
        if tags != ntags:
          ...
          osmcode.write( '<way id="%s" uid="%s" user="%s" timestamp="%s" version="%s" action="modify">\n'%(items["id"], items.get("uid",0), xml_escape((items.get("user", "unknown"),))[0], items["timestamp"],  items["version"]))
  ...
  for action, elem in context:
    ...
    if elem.tag == "node":
      if items['id'] in nodes_needed and items['id'] not in nodes_out:
        ...
        nodes_out.add(items['id'])
        osmcode.write( '<node id="%s" uid="%s" user="%s" timestamp="%s" lon="%s" lat="%s" version="%s">\n'%(items["id"], items.get("uid",0), xml_escape((items.get("user", "unknown"),))[0], items["timestamp"], items["lon"],items["lat"],items["version"]))
