    ...
    if elem.tag == "node":
      if tags:
        ...
        if tags != ntags:
          ...
          osmcode.write( '<node id="%s" uid="%s" user="%s" changeset="%s" timestamp="%s" lon="%s" lat="%s" version="%s" action="modify">\n'%(items["id"], items["uid"], xml_escape((items["user"],))[0], items["changeset"], items["timestamp"], items["lon"],items["lat"],items["version"]))
      ...
      if tags:
        ...
        if tags != ntags:
          ...
          osmcode.write( '<way id="%s" uid="%s" user="%s" timestamp="%s" changeset="%s" version="%s" action="modify">\n'%(items["id"], items["uid"], xml_escape((items["user"],))[0], items["timestamp"], items["changeset"], items["version"]))
